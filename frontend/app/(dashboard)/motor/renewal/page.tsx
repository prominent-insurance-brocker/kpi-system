...
const columns = [
  { key: 'date', header: 'Date', render: (item: MotorRenewalEntry) => formatDate(item.date) },
  { key: 'quotations', header: 'Quotations' },
  { key: 'retention', header: 'Retention' },
  { key: 'tat', header: 'TAT' },
  { key: 'accuracy', header: 'Accuracy', render: (item: MotorRenewalEntry) => `${item.accuracy}%` },
  { key: 'added_by_name', header: 'Added By' },
  { key: 'added_at', header: 'Added At', render: (item: MotorRenewalEntry) => formatDateTime(item.added_at) },
];
...
export default function MotorRenewalPage() {
  ...
  const [entries, setEntries] = useState<MotorRenewalEntry[]>([]);
  const [totalCount, setTotalCount] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingEntry, setEditingEntry] = useState<MotorRenewalEntry | null>(null);
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const result = await fetchApi<{ results: MotorRenewalEntry[]; count: number }>(`/api/entries/motor-renewal/?${params}`);
  ...
  const handleSave = async (formData: Partial<MotorRenewalEntry>) => {
    setError('');
    const endpoint = editingEntry ? `/api/entries/motor-renewal/${editingEntry.id}/` : `/api/entries/motor-renewal/`;
    const result = await fetchApi<MotorRenewalEntry>(endpoint, { method: editingEntry ? 'PATCH' : 'POST', body: JSON.stringify(formData) });
    if (result.data) { setIsModalOpen(false); setEditingEntry(null); fetchEntries(); }
    else { setError(result.error || 'Failed to save entry'); }
  };

  const handleDelete = async (entry: MotorRenewalEntry) => {
    if (!confirm('Are you sure you want to delete this entry?')) return;
    const result = await fetchApi<void>(`/api/entries/motor-renewal/${entry.id}/`, { method: 'DELETE' });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Motor Renewal</h1><p className="text-muted-foreground">Manage motor renewal quotations</p></div>
...
function EntryForm({ entry, onSave, onClose, error }: { entry: MotorRenewalEntry | null; onSave: (data: Partial<MotorRenewalEntry>) => void; onClose: () => void; error: string }) {
  const [formData, setFormData] = useState({ date: '', quotations: '', retention: '', tat: '', accuracy: '' });
  const [isSubmitting, setIsSubmitting] = useState(false);
  useEffect(() => {
    if (entry) setFormData({ date: entry.date, quotations: String(entry.quotations), retention: String(entry.retention), tat: String(entry.tat), accuracy: String(entry.accuracy) });
    else setFormData({ date: new Date().toISOString().split('T')[0], quotations: '', retention: '', tat: '', accuracy: '' });
  }, [entry]);
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setIsSubmitting(true); await onSave({ date: formData.date, quotations: Number(formData.quotations), retention: Number(formData.retention), tat: Number(formData.tat), accuracy: Number(formData.accuracy) }); setIsSubmitting(false); };
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="space-y-2"><Label>Retention</Label><Input type="number" min="0" placeholder="Enter retention" value={formData.retention} onChange={(e) => setFormData({ ...formData, retention: e.target.value })} required /></div>
