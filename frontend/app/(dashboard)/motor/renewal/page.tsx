...
function EntryForm({ entry, onSave, onClose, error }: { entry: MotorRenewalEntry | null; onSave: (data: Partial<MotorRenewalEntry>) => void; onClose: () => void; error: string }) {
  const [formData, setFormData] = useState({ date: '', quotations: 0, retention: 0, tat: 0, accuracy: 0 });
  const [isSubmitting, setIsSubmitting] = useState(false);
  useEffect(() => {
    if (entry) setFormData({ date: entry.date, quotations: entry.quotations, retention: entry.retention, tat: entry.tat, accuracy: Number(entry.accuracy) });
    else setFormData({ date: new Date().toISOString().split('T')[0], quotations: 0, retention: 0, tat: 0, accuracy: 0 });
  }, [entry]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="space-y-2"><Label>Retention</Label><Input type="number" min="0" value={formData.retention} onChange={(e) => setFormData({ ...formData, retention: Number(e.target.value) })} required /></div>
