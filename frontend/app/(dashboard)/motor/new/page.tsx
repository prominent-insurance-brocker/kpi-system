...
export default function MotorNewPage() {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/entries/motor-new/?${params}`, { credentials: 'include' });
  ...
  const handleSave = async (formData: Partial<MotorNewEntry>) => {
    setError('');
    const url = editingEntry ? `${API_BASE_URL}/api/entries/motor-new/${editingEntry.id}/` : `${API_BASE_URL}/api/entries/motor-new/`;
  ...
  const handleDelete = async (entry: MotorNewEntry) => {
    ...
    const response = await fetch(`${API_BASE_URL}/api/entries/motor-new/${entry.id}/`, { method: 'DELETE', credentials: 'include' });
