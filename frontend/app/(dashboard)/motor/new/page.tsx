...
export default function MotorNewPage() {
  ...
  const [entries, setEntries] = useState<MotorNewEntry[]>([]);
  const [totalCount, setTotalCount] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingEntry, setEditingEntry] = useState<MotorNewEntry | null>(null);
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const result = await fetchApi<{ results: MotorNewEntry[]; count: number }>(`/api/entries/motor-new/?${params}`);
  ...
  const handleSave = async (formData: Partial<MotorNewEntry>) => {
    setError('');
    const endpoint = editingEntry ? `/api/entries/motor-new/${editingEntry.id}/` : `/api/entries/motor-new/`;
    const result = await fetchApi<MotorNewEntry>(endpoint, { method: editingEntry ? 'PATCH' : 'POST', body: JSON.stringify(formData) });
    if (result.data) { setIsModalOpen(false); setEditingEntry(null); fetchEntries(); }
    else { setError(result.error || 'Failed to save entry'); }
  };

  const handleDelete = async (entry: MotorNewEntry) => {
    if (!confirm('Are you sure you want to delete this entry?')) return;
    const result = await fetchApi<void>(`/api/entries/motor-new/${entry.id}/`, { method: 'DELETE' });
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Motor New</h1><p className="text-muted-foreground">Manage motor new quotations</p></div>
